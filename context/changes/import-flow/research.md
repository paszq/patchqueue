...
powstała wcześniej (`e81f9da`, cała ścieżka główna) i nigdy nie została do nich
...
- **Parsowanie jest czyste** — bez bazy, HTTP i zegara — więc 28 testów pokrywa je tabelą
  przypadków zamiast klikaniem. Trzy z nich złapały prawdziwe błędy (commit `45ec838`):
...
- `commit 45ec838` — wprowadzenie warstwy tłumaczącej wraz z opisem trzech błędów złapanych
