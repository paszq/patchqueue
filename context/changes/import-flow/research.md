...
powstała wcześniej (`69f8357`, cała ścieżka główna) i nigdy nie została do nich
...
- **Parsowanie jest czyste** — bez bazy, HTTP i zegara — więc 28 testów pokrywa je tabelą
  przypadków zamiast klikaniem. Trzy z nich złapały prawdziwe błędy (commit `a29d7eb`):
...
- `commit a29d7eb` — wprowadzenie warstwy tłumaczącej wraz z opisem trzech błędów złapanych
