...
Commit: `66946d8`, uzasadnienie: `context/changes/account-isolation/change.md`.
...
Cała ścieżka główna S-01…S-08: commit `e5774fb`.
...
_Kandydat 1 — rozstrzygnięcie jako jedna operacja atomowa_ (`d9156ed`). `recordDecision`
...
_Kandydat 2 — podział warstwy danych wzdłuż pojęć domenowych_ (`1464efc`). Jeden plik
...
`10f9501`. Odpowiedź na kandydata #1 z destylacji. Jeden z jego niezmienników — „zmiana
...
`e2e` → `deploy`. Commit `09eccb1`, sekrety `642a883`.

**Ciągłe wdrażanie z weryfikacją żywej instancji** (`86fb76a`) — po `wrangler deploy`
...
**Strażnik przed cichym pomijaniem testów** (`3751e68`) — opisany w następnej sekcji,
...
z ostrzeżeniem. Commit `3751e68` był jednocześnie sprawdzianem — jeśli testy pomijały się
...
Uzasadnienie: `context/changes/atomic-decisions/change.md`, naprawa: `d9156ed`.
